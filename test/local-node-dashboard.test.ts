...
test("Providers dashboard exposes complete local-node pairing and management controls", async () => {
  ...
  for (const behavior of [
    ...
    "local-node:", "routableProviders", "data-local-toggle-settings", "data-local-open-playground",
    "renderLocalRoutingSettings", "/api/playground/local", "localNodeInitials",
  ]) {
    assert.match(app, new RegExp(behavior.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  }
  assert.match(app, /npx @free-llm-router\/cli/);
  ...
  assert.match(css, /\.local-routing-settings/);
  assert.match(vercel, /\/api\/local-nodes\/:path\*/);
});

test("CLI and documentation expose the complete local Ollama lifecycle", async () => {
  const [cli, service, packageJson, guide, security, troubleshooting, releaseChecklist] = await Promise.all([
  ...
  assert.match(guide, /Settings → Router & Policies → Routing Policies → Local routing/);
  ...
  assert.match(cli, new RegExp(`LOCAL_NODE_CLI_VERSION = ["']${version.replaceAll(".", "\\.")}["']`));
  assert.match(cli, /"message" in payload\.error/);
